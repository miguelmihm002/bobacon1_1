...
from bobacon.common_helper import create_float_metavar, create_int_metavar, get_last
from bobacon.execution import get_available_execution_providers
...
def create_config_path_program() -> ArgumentParser:
	...
	group_paths.add_argument('--config-path', help = wording.get('help.config_path'), default = 'bobacon.ini')
...
def create_face_detector_program() -> ArgumentParser:
	...
	group_face_detector.add_argument('--face-detector-model', help = wording.get('help.face_detector_model'), default = config.get_str_value('face_detector.face_detector_model', 'yoloface'), choices = bobacon.choices.face_detector_models)
	known_args, _ = program.parse_known_args()
	face_detector_size_choices = bobacon.choices.face_detector_set.get(known_args.face_detector_model)
...
def create_face_masker_program() -> ArgumentParser:
	...
	group_face_masker.add_argument('--face-occluder-model', help = wording.get('help.face_occluder_model'), default = config.get_str_value('face_detector.face_occluder_model', 'xseg_1'), choices = bobacon.choices.face_occluder_models)
	group_face_masker.add_argument('--face-parser-model', help = wording.get('help.face_parser_model'), default = config.get_str_value('face_detector.face_parser_model', 'bisenet_resnet_34'), choices = bobacon.choices.face_parser_models)
...
def create_uis_program() -> ArgumentParser:
	program = ArgumentParser(add_help = False)
	available_ui_layouts = [ file.get('name') for file in list_directory('bobacon/uis/layouts') ]
...
def create_download_providers_program() -> ArgumentParser:
	program = ArgumentParser(add_help = False)
	download_providers = list(bobacon.choices.download_provider_set.keys())
	group_download = program.add_argument_group('download')
	group_download.add_argument('--download-providers', help = wording.get('help.download_providers').format(choices = ', '.join(download_providers)), default = config.get_str_list('download.download_providers', ' '.join(bobacon.choices.download_providers)), choices = download_providers, nargs = '+', metavar = 'DOWNLOAD_PROVIDERS')
...
def create_download_scope_program() -> ArgumentParser:
	program = ArgumentParser(add_help = False)
	group_download = program.add_argument_group('download')
	group_download.add_argument('--download-scope', help = wording.get('help.download_scope'), default = config.get_str_value('download.download_scope', 'lite'), choices = bobacon.choices.download_scopes)
...
def create_misc_program() -> ArgumentParser:
	program = ArgumentParser(add_help = False)
	log_level_keys = list(bobacon.choices.log_level_set.keys())
