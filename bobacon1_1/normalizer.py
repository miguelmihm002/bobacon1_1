...
from bobacon1_1.filesystem import is_directory
from bobacon1_1.typing import Padding, Fps
...
def normalize_output_path(target_path : Optional[str], output_path : Optional[str]) -> Optional[str]:
	if target_path and output_path:
		...
		if is_directory(output_path):
			output_hash = hashlib.sha1(str(bobacon1_1.globals.__dict__).encode('utf-8')).hexdigest()[:8]
