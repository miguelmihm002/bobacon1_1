...
from bobacon import face_detector, state_manager, wording
...
def render() -> None:
	...
	with gradio.Row():
		FACE_DETECTOR_MODEL_DROPDOWN = gradio.Dropdown(
			...
			choices = bobacon.choices.face_detector_models,
...
def update_face_detector_model(face_detector_model : FaceDetectorModel) -> Tuple[gradio.Dropdown, gradio.Dropdown]:
	...
	if face_detector.pre_check():
		face_detector_size_choices = bobacon.choices.face_detector_set.get(state_manager.get_item('face_detector_model'))
